...
describe('ProductOrder e2e test', () => {
  ...
  const productOrderSample = { placedDate: '2023-01-18T11:49:04.294Z', status: 'CANCELLED', code: 'Buckinghamshire' };
  ...
  describe('new ProductOrder page', () => {
    ...
    it('should create an instance of ProductOrder', () => {
      cy.get(`[data-cy="placedDate"]`).type('2023-01-18T06:00').blur().should('have.value', '2023-01-18T06:00');

      cy.get(`[data-cy="status"]`).select('PENDING');

      cy.get(`[data-cy="invoiceId"]`).type('89147').should('have.value', '89147');

      cy.get(`[data-cy="code"]`).type('Bacon known c').should('have.value', 'Bacon known c');
